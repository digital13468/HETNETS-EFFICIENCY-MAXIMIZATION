...
#define map_x 40000 //x-axis of map
#define map_y 35000	//y-axis of map
#define Ntp 1540	//number of TPs
...
#define rbs 300	//the radius of a micro BS in meter
#define MNbs 34	//assumed maximum number of BSs including micro and macro BSs
#define MBR 990	//maximum bandwidth requirement QPSK 3/4
#define mNbs 19	//number of micro BSs
//table:parameters per channel bandwidth
#define BW 3333333	//10 MHz channel bandwidth
...
#define DSt 15000	//total resource blocks in 1000 DL subframe within 1 second 
...
#define MP 40		//UNIT 1mW=30dBm maximum power of a BS: 20W
#define SBP 8.00	//Power for BS in sleep mode
#define ABP 500.0	//Basic power for BS in active mode
#define BP 0.07	//UNIT(%) BLOCKING PROBABILITY
#define mP 0.14	//maximum transmission power of micro BS: 10W
#define mSBP 0.028	//Power for micro BSs in sleep mode
#define mABP 5.0	//Basic power for micro BSs in active mode
...
#define X2suburban 1750
#define Y1suburban 500
#define Y2suburban 1750
...
#define Modulation 4	//number of modulation schemes
#define MaxCellSize 7500.0	//the maximum macro cell size
#define maxCellSize 1000	//the maximum micro cell size
...
double* ptrP;
 ...
 //double Mrr[_r][_r];
 //double Mrt[_r][_t];
void checkAdjacentBSs(int* Xbs, int* Ybs, int* Xtp, int* Ytp, int Tbs, int* ptrAdjacenBSs);
...
*/nUser=(Tbs-mNbs)*PoissonRandomNumber(lambda,maxAUser);
...
//  SmallUser(Xtp,Ytp,Px,Py,Tbs,Xbs,Ybs,nUser,mNbs,space,maxCellSize,Rbs,Ntp);
 checkAdjacentBSs(Xbs,Ybs,Xtp,Ytp,Tbs,ptrAdjacentBSs);
...
void DataSubcarriers(int* ptrDS, double* ptrBR, double* ptrDP){
	...
	for(j=0;j<Ntp;j++)
		for(k=0;k<Modulation;k++)
			  DS[j][k]=ceil(((1/N/3*9/8/4*5*FFT)/DP[j][k]*BR[j]/10)/12);
...
return;
   ...
   //*******************************************************
void checkAdjacentBSs(int* Xbs, int* Ybs, int* Xtp, int* Ytp, int Tbs, int* AdjacentBSs) {
 // In this code, the candidate serving BSs for users are determined
 int i,j;
 
for(i=0;i<Tbs;i++)
	for(j=0;j<Ntp;j++)
		*(AdjacentBSs+i*Ntp+j)=0;
 for(i=0;i<Tbs-mNbs;i++)
	for(j=0;j<Ntp;j++){
