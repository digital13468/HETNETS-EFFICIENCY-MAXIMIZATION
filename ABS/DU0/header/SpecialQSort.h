...
#ifndef _SPECIAL_QSORT_H
#define _SPECIAL_QSORT_H

int * DoubleIndex_q_sort(double number[], int left, int right, int MAX, int *index){//pass the data array, the starting index and ending index of the data array, and the size of the array
...
//start_tick = clock();
           ...
           DoubleIndex_quickSort(number,left,right,MAX,index);
           /*for(i=0;i<MAX;i++)
           printf("%d ",number[i]);
           */
...
DoubleIndex_quickSort(double number[], int left, int right, int MAX,int index[]) {  
     if(left < right) {         
             int i = left;         
             int j = right + 1;  
             int m;       
             ...
             /*printf("SWAP %d %d index %d %d\n",number[left],number[j],index[j],index[left]);
              printf("outer new array:");
                     ...
                     printf("outer new index array:");
                      ...
                      printf("%d ",index[m]);        
                      printf("\n");*/
             DoubleIndex_quickSort(number, left, j-1,MAX,index);            
             DoubleIndex_quickSort(number, j+1, right,MAX,index);       
             } 
     } 
