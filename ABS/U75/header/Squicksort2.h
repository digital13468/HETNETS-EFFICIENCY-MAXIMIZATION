...
#ifndef _SQUICKSORT2_H
#define _SQUICKSORT2_H

int * Sq_sort(double number[], int left, int right, int MAX){//pass the data array, the starting index and ending index of the data array, and the size of the array
...
//start_tick = clock();
           ...
           SquickSort(number,left,right,MAX,index);
           /*for(i=0;i<MAX;i++)
           printf("%d ",number[i]);
           */
...
SquickSort(double number[], int left, int right, int MAX,int index[]) {  
     if(left < right) {         
             int i = left;         
             int j = right + 1;  
             int m;       
             ...
             /*printf("SWAP %d %d index %d %d\n",number[left],number[j],index[j],index[left]);
              printf("outer new array:");
                     ...
                     printf("outer new index array:");
                      ...
                      printf("%d ",index[m]);        
                      printf("\n");*/
             SquickSort(number, left, j-1,MAX,index);            
             SquickSort(number, j+1, right,MAX,index);       
             } 
     } 
