...
switchOF(int Tbs, int* DS, int* minM, int nUser, int Modulation, int Ntp, int* ptrAdjacentBSs, double mSBP, double mABP, double* P, double ABP, double SBP, int mNbs, double BP, double MP, double mP, int DSt, double* BR, int* ConnectionAssignment){
	FILE *PrintToFile;
	if((PrintToFile=fopen("switch.txt","w"))==NULL)
		printf("\nerror!Fail to open switch.txt!");
	else
		printf("\nOpen switch.txt successfully!\n");
	fprintf(PrintToFile,"This is the input to CPLEX for no BS switch-off.\n");
