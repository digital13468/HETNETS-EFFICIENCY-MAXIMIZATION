...
MCS(int Tbs, int* DS, int* minM, int nUser, int Modulation, int Ntp, int* ptrAdjacentBSs, double mSBP, double mABP, double* P, double ABP, double SBP, int mNbs, double BP, double MP, double mP, int DSt, double* BR, int* UserModulation){
	FILE *PrintToFile;
	if((PrintToFile=fopen("MCS.txt","w"))==NULL)
		printf("\nerror!Fail to open MCS.txt!");
	else
		printf("\nOpen MCS.txt successfully!\n");
	fprintf(PrintToFile,"This is the input to CPLEX for default MCS.\n");
