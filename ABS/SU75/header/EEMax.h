...
EEMax(int Tbs, int* DS, int* minM, int nUser, int Modulation, int Ntp, int* ptrAdjacentBSs, double mSBP, double mABP, double* P, double ABP, double SBP, int mNbs, double BP, double MP, double mP, int DSt, double* BR, int* ConnectionAssignment){
	FILE *PrintToFile;
	if((PrintToFile=fopen("EEMax.txt","w"))==NULL)
		printf("\nerror!Fail to open EEMax.txt!");
	else
		printf("\nOpen EEMax.txt successfully!\n");
	fprintf(PrintToFile,"This is the input to CPLEX for Energy Efficiency Optimization Model.\n");
